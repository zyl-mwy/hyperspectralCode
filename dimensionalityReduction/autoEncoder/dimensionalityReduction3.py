...
#工具函数，为图像加噪声
def add_noise(imgs, thresh=2): #imgs:bs,28*28
    imgs_copy = imgs.clone()
    mask = torch.randn_like(imgs) #mask: bs,28*28
    #越过阈值的位置，数据填充为噪声（椒盐噪声）
    batch0,index0 = torch.where(mask<-thresh)
    for i,j in zip(batch0,index0): imgs_copy[i,j] = 0
    batch1,index1 = torch.where(mask>thresh)
    for i,j in zip(batch1,index1): imgs_copy[i,j] = 1
    return imgs_copy
...
for epoch in range(epochs):   
    ...
    for imgs, lbls in t: #imgs:(bs,28,28)
        ...
        #数据加噪声
        ns_imgs = add_noise(imgs)
        #模型运算     
        dns_imgs = model(ns_imgs)
        #计算损失
        loss = loss_BCE(dns_imgs, imgs) # 降噪后重构与原始数据的差距(也可使用loss_MSE)
        #反向传播、参数优化，重置
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
    ...
    for imgs, label in e:
        ...
        dns_imgs = model(ns_imgs)
        #计算损失
        loss = loss_BCE(dns_imgs, imgs) # 降噪后重构与原始数据的差距
    ...
    #展示效果   
    #将测试步骤中的数据、重构数据绘图
    concat = torch.cat((ns_imgs[0].view(28, 28),
            dns_imgs[0].view( 28, 28),imgs[0].view(28,28)), 1)
...
#取一组数据
imgs = dataset[0][0].view(1,-1) #raw: bs,28,28->bs,28*28
#加椒盐噪声
ns_imgs = add_noise(imgs)
#将加噪声后图像经过模型处理
dns_imgs = model(ns_imgs)
